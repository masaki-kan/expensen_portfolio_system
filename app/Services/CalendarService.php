...
class CalendarService
{
  ...
  public function getWeeks($user_id)
  {
    ...
    for ($day = 1; $day <= $days_in_month; $day++, $day_of_week++) {
      ...
      $user = Train::where('user_id', $user_id)->where('date', 'LIKE', $train_date . '%')->where('subject', 0)->exists();
      //カレンダーの日付がデータベースにあるかないか
      if ($user == true) {
        //あったら●
        $week .= '<td style="background: #fdfb9c;">' . $day . '<br><a href="/caldender_detail/' . $train_date . '"><i class="far fa-circle"></i></a>';
      } else {
        //なかったら✖️
